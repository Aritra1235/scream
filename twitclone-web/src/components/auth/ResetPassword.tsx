...
const apiBase = normalizedBaseUrl ? `${normalizedBaseUrl}/${apiPrefix}` : `/api/${apiPrefix}`;

type ResetStatus = "idle" | "verifying" | "ready" | "submitting" | "success" | "error" | "invalid";

const statusStyles: Record<ResetStatus, { label: string; accent: string; icon: ReactNode }> = {
  idle: {
...
export default function ResetPassword() {
  ...
  const [statusMessage, setStatusMessage] = useState(
    tokenFromParams ? "Checking your reset link..." : "Open the reset link from your email to continue."
  );
  ...
  useEffect(() => {
    if (errorCode) {
      setStatus((prev) => (prev === "success" || prev === "submitting" ? prev : "invalid"));
      setStatusMessage("This reset link is no longer valid. Request a fresh one.");
      return;
    }

    if (!token) {
      setStatus((prev) => (prev === "success" || prev === "submitting" ? prev : "idle"));
    ...
    setStatus((prev) => (prev === "success" || prev === "submitting" ? prev : "verifying"));
    ...
    const verifyToken = async () => {
      try {
        ...
        if (result.valid) {
          setStatus((prev) => (prev === "success" || prev === "submitting" ? prev : "ready"));
        ...
        if (aborted) return;
        setStatus((prev) => (prev === "success" || prev === "submitting" ? prev : "error"));
        setStatusMessage("We couldn't verify your link. Refresh the page or request a new email.");
      }
    };
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    if (!token) {
      ...
      setStatus("invalid");
      setStatusMessage("We couldn't find a valid token. Request a new link below.");
      return;
    }
    ...
    try {
      ...
      if (result.error) {
        setStatus("error");
        setFormError(result.error.message || "We couldn't update your password. Try again.");
        setStatusMessage(result.error.message || "We couldn't update your password. Try again.");
        return;
      }
  ...
  const canShowForm = status === "ready" || status === "submitting" || status === "success";
        ...
        <Card className="flex-1 border-border/40 bg-card/80 backdrop-blur">
          ...
          <CardContent className="space-y-8">
            <div className="rounded-2xl border border-border/50 bg-background/60 p-6 shadow-inner">
                ...
                <div className="flex-1">
                  ...
                  {status !== "invalid" && (
                    <p className="mt-2 text-sm text-muted-foreground">
                      The link works only once and expires quickly. If it fails, request a fresh email.
                    </p>
                  )}
                </div>
              </div>
              {status === "invalid" && (
                <div className="mt-6 rounded-xl bg-destructive/10 px-4 py-3 text-sm text-destructive">
                  This link is invalid or expired. Request another reset email to continue.
                </div>
              )}
              {status === "error" && formError && (
                <div className="mt-6 rounded-xl bg-destructive/10 px-4 py-3 text-sm text-destructive">{formError}</div>
              )}
              {status === "success" && (
            ...
            <div className="rounded-2xl border border-border/40 bg-background/40 p-6 shadow-sm">
              <p className="text-sm font-semibold text-foreground">Tips for a strong password</p>
            ...
            <div className="flex flex-wrap items-center gap-3">
              <Button variant="outline" className="border-border text-foreground hover:bg-accent" onClick={() => router.push("/forgot-password")}>
                Request a new link
              </Button>
              <p className="text-sm text-muted-foreground">
                Still stuck? Contact <span className="font-medium text-foreground">support@scream.aritra.ovh</span>.
              </p>
            </div>
          </CardContent>
        </Card>
        ...
        <div className="w-full max-w-lg space-y-6">
          {canShowForm ? (
            <Card className="border-border/40 bg-card/80 backdrop-blur">
              <CardHeader>
                <CardTitle className="text-xl font-semibold text-foreground">Choose a new password</CardTitle>
                <CardDescription>Make sure it&rsquo;s unique to SCREAM.</CardDescription>
              </CardHeader>
              <CardContent>
                <form className="space-y-4" onSubmit={handleSubmit}>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      ...
                      >
                        {showConfirm ? <EyeOff className="h-4 w-4 text-foreground" /> : <Eye className="h-4 w-4 text-foreground" />}
                      </Button>
                    </div>
                  </div>
          ...
          {status === "success" && (
            <Card className="border border-border/40 bg-background/60">
              <CardContent className="py-4 text-sm text-muted-foreground">
                You&rsquo;re all set. We&rsquo;ll send you to the sign in page automatically, or{" "}
                <button className="font-medium text-foreground underline" onClick={() => router.push("/sign-in")}>
                  go now
                </button>
                .
              </CardContent>
